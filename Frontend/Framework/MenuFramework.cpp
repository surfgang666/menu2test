...
bool ui::SingleSelect(const char* label, int* current_item, std::vector<std::string> items) {
	*current_item = std::clamp(*current_item, 0, int(items.size() - 1));
	...
	if (ui::BeginCombo(label, items.at(*current_item).c_str(), items.size())) {
...
bool ui::BeginListbox(const char* id, const Vec2 Size, GuiFlags flags) {
	...
	if (Size == Vec2(0, 0)) {
		...
		//Rect Fullbb = { Window->CursorPos + Vec2(20, 0), Vec2(std::clamp(Window->Size.x - 101.f, 64.f, 200.f), 126.f) };
		Rect Fullbb = { Window->CursorPos + Vec2(20, 0), Vec2(std::clamp(Window->Size.x - 101.f, 64.f, 200.f), 426.f) };
